...
const ApiServices = {
	...
	query(endpoint, params) {
		return Vue.axios.get(endpoint, params).catch((error) => {
			if (error && error.response)
				throw new Error(
					`DigitalTolk ApiService on API:${endpoint} with ${params}, error: ${error.data.message}`
				);
			else
				throw new Error(
					`DigitalTolk ApiService on API:${endpoint} with ${params}, error: ${error}`
				);
		});
	},
	/* General get request */
	get(endpoint, params) {
		...
		return api.get(params).catch((error) => {
			if (error && error.data)
				throw new Error(
					`DigitalTolk ApiService on API: ${params}, error: ${error.data.message}`
				);
			else
				throw new Error(
					`DigitalTolk ApiService on API: ${params}, error: ${error}`
				);
		});
	},
